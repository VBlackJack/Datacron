...
règle dont le tag vit dans le `subject_namespace` de la politique. Sans politique, ou sans
espace de noms de sujet, il n'y a pas de dossier de sujet et ni `NO_STATE_NOTE` ni
`UNLINKED` n'existent. Seules comptent les notes gouvernées qui sont réellement dans le
dossier de la règle ; une note mal placée est déjà `WRONG_FOLDER`.
...
| `counts` | Nombre d'écarts par nature, les neuf natures dans l'ordre déclaré |
...
L'identité `scanned = governed + unmatched` est toujours vraie. Un écart de dossier
(`NO_STATE_NOTE`) a le dossier pour `rel_path` ; il ne change aucun des trois compteurs.
