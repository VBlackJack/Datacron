...
The tool description repeats the record schema clause by clause (required fields, identifier,
hash and ULID patterns, enumerations, text bounds, nullable fields and defaults) plus the
runtime limit on records per call, because some MCP clients present the schema without its
definitions or patterns; the schema itself is unchanged.
